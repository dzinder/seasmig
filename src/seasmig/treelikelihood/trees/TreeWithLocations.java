...
public class TreeWithLocations implements LikelihoodTree {
	...
	public TreeWithLocations(TransitionModel createTreeModel, int numNodes, Config config) {
		...
		int rootLocation = getRandomSampleFrom(createTreeModel.rootfreq(root.time));
	...
	public TreeWithLocations(TransitionModel createTreeModel, jebl.evolution.trees.SimpleRootedTree tree, HashMap<String,Integer> taxaIndices_, List<Taxon> taxa_, Config config, double lastTipTime) {
		this.config = config;
		...
		if (config.seqModelType==SeqModelType.NONE)
			root = new TreeWithLocationsNode(null, UNKNOWN_LOCATION,rootTaxonIndex,0,null,false);
		else
			root = new TreeWithLocationsNode(null, UNKNOWN_LOCATION,rootTaxonIndex,0,null,true);
	...
	public void fillRandomTraits() {
		root.setLoc(getRandomSampleFrom(migrationModel.rootfreq(root.time)));
	...
	// For test purpose
	public String printTaxaAndLocation() {
		String returnValue="";
		for (TreeWithLocationsNode node : root) {
			if (node.taxonIndex!=(-1)) {
				returnValue+=(taxa.get(node.getTaxonIndex()).getName()+"\n"+node.getLoc())+"\n";
			}
		}
		return returnValue;
	}
